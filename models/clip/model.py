...
class AttentionPool2d(nn.Module):
    ...
    def forward(self, x):
        x = x.flatten(start_dim=2).permute(2, 0, 1)  # NCHW -> (HW)NC #DDD
        x = torch.cat([x.mean(dim=0, keepdim=True), x], dim=0)  # (HW+1)NC
        x = x + self.positional_embedding[:, None, :].to(x.dtype)  # (HW+1)NC
        x, _ = F.multi_head_attention_forward(
            query=x[:1], key=x, value=x,#DDD
...
class ResidualAttentionBlock(nn.Module):
    def __init__(self, d_model: int, n_head: int, attn_mask: torch.Tensor = None, add_prompt=False,
        ...
        self._CG_tuning = False
        if i < design_details['CG']['stack_layers']:
            self._CG_tuning = True
            
        if add_prompt:
            if text_layer:
                self.attn = MultiheadAttention(d_model, n_head, design_details, self._CG_tuning, prefix_pool_size=design_details["pool_size"], 
                                               prefix_len=design_details['RG']["language_ctx"])
            else:
                self.attn = MultiheadAttention(d_model, n_head, design_details, self._CG_tuning, prefix_pool_size=design_details["pool_size"], 
                                               prefix_len=design_details['RG']["vision_ctx"])
        else:
            self.attn = MultiheadAttention(d_model, n_head, design_details, self._CG_tuning)
...
class Transformer(nn.Module):
    def __init__(self, width: int, layers: int, heads: int, attn_mask: torch.Tensor = None, prompts_needed=0,
        ...
        self.resblocks = nn.Sequential(*[ResidualAttentionBlock(width, heads, attn_mask, True,
                                                                        text_layer, i,
                                                                        design_details) if prompts_needed > i #决定在多少注意力层添加额外prompt
...
class VisionTransformer(nn.Module):
    def __init__(self, input_resolution: int, patch_size: int, width: int, layers: int, heads: int,
        ...
        self.prompt_till_layer_visual = design_details["RG"]["vision_stack_layers"]
        ...
        self.prompt = nn.Parameter(torch.randn(design_details['prompt_length'],design_details['num_features'])) #VL Prompt 768
    ...
    def forward(self, x: torch.Tensor, prompt_ids=None, batch_weight=None):
        ...
        pos_vp = self.prompt + self.positional_embedding[0].expand(self.prompt.shape[0], -1) # add vision prompt
        pos_vp = pos_vp.expand(x.shape[0],-1,-1)
        x = torch.cat([x[:,0].unsqueeze(1), pos_vp, x[:,1:]],dim=1)#
...
class CLIP(nn.Module):
    def __init__(self,
        ...
        if isinstance(vision_layers, (tuple, list)):
            ...
            vision_heads = vision_width // 64 # vision_width is dim, specifically 1024
        ...
        # hyper-parameter if need to add prompt embeddings inside to the input
        # of transformer block or not:
        prompt_till_layer_text = design_details["RG"]['language_stack_layers']
    ...
    def encode_image(self, image, prompt_ids):
        #we newly add prompt_ids to choise learnable prompt in pool
        return self.visual(image.type(self.dtype), prompt_ids)
